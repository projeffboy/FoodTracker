...
def get_food_info(food):
    ...
    for food_nutrient in food_nutrients:
        ...
        if(nutrient_id==Protien.get_nutrientId()):
            food.set_protien(float(food_nutrient["amount"]))
        # get fat
        if(nutrient_id==Fat.get_nutrientId()):
            food.set_fat(float(food_nutrient["amount"]))
        # get sugar
        if(nutrient_id in Sugar.get_nutrientId()[:3]):
            food.set_sugar(food.get_sugar() + float(food_nutrient["amount"]))
        elif(nutrient_id == Sugar.get_nutrientId()[3]):
            food.set_sugar(float(food_nutrient["amount"]))
        # get fiber
        if(nutrient_id==Fiber.get_nutrientId()):
            food.set_fiber(float(food_nutrient["amount"]))
        # get calcuim
        if(nutrient_id==Calcium.get_nutrientId()):
            food.set_calcium(float(food_nutrient["amount"]))
        # get sodium
        if(nutrient_id==Sodium.get_nutrientId()):
            food.set_sodium(float(food_nutrient["amount"]))
    if (energy != -1):
